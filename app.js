...
const activateLightBox = (thisElmt) => {
  ...
  //라이트 박스가 활성화 될 때 클릭된 이미지 요소의 src 속성 값을 읽어 라이트 박스이미지src 속성값으로전달
...
//이미지 요소 배열을 순회한다. 첫번째 파라미터는 이미지 요소 배열, 두번째 파라미터 boolean 값-true일 경우? 뒤에 : 을 기준으로 앞에 로직이 적용되고 , false 일 경우 뒤에 로직이적용된다.
...
function showItems(thisItem) {
  // 탭 버튼을 보튼을 클릭 했을때  클릭된 탭의 data-filter 속성값 저장
  ...
  //클릭한 탭 버튼 요소의 data-filter 속성값과 이미지 요소의 data-filter속성값이일치하는 것만 필터링
  const thisImage = Array.from(images).filter(
    //Array.from() : 유사배열 (NodeList, HTMLCollection)을 정식 배열로전환
...
btns.forEach((btn) => {
  ...
  btn.addEventListener("click", function () {
    ...
    //이미지 요소들이 재배치 될때 0.1 초 뒤에 showItems 함수 실행
    setTimeout(() => {
      showItems(this);
    }, 100);
  });
...
function closeLightBox() {
  ...
  overlay.classList.remove("show"); // 배경오버레이를 없앤다
}

// 이벤트 실행 함수를 전달할 때 ()를 붙이지 않는다.
closeBtn.addEventListener("click", closeLightBox); // x 버튼을 클릭했을때 함수 실행
overlay.addEventListener("click", closeLightBox); //배경 오버레이를 클릭 했을때 함수실행
